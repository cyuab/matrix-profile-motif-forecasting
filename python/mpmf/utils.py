...
def get_top_1_motif(T, m, l=1, include_itself = False):
    ...
    for i in range(first_i_idx, len(T)):
        ...
        if j >= 0:  # We have found a top-1 motif of q
            ...
            top_1_motif_idx_delta[i] = q_idx - top_1_motif_idx[i]  # how far the top-1 motif is from the query.
            query = T[q_idx: q_idx + m].to_numpy()  # The distance calculation requires they are in NumPy arrays.
            top_1_motif = T[j: j + m].to_numpy()
            top_1_motif_dist[i] = np.linalg.norm(stumpy.core.z_norm(query) - stumpy.core.z_norm(top_1_motif))
            top_1_motif_point_after_cols = {f"top_1_motif_point_after_{ll+1}": top_1_motif_points_after[:, ll] for ll in range(l)}
...
def get_top_k_motifs(T, m, k=1, l=1, include_itself = False):
    ...
    for i in range(first_i_idx, len(T)):
        ...
        for kk in range(len(matches)):
            ...
            top_k_motif_points_after[i, kk, :] = T[matches[kk, 1] + m : matches[kk, 1] + m + l]
            
    top_k_motif_dist_cols = {f"top_{kk+1}_motif_dist": top_k_motif_dist[:, kk] for kk in range(k)}
    top_k_motif_idx_cols = {f"top_{kk+1}_motif_idx": top_k_motif_idx[:, kk] for kk in range(k)}
    top_k_motif_idx_delta_cols = {f"top_{kk+1}_motif_idx_delta": top_k_motif_idx_delta[:, kk] for kk in range(k)}
    top_k_motif_point_after_cols ={}
    for kk in range(k):
        top_k_motif_point_after_cols.update({f"top_{kk+1}_motif_point_after_{ll+1}": top_k_motif_points_after[:, kk, ll] for ll in range(l)})
    df_motif = pd.DataFrame({
